...
def run_query(query_str, params=None, ttl="5m"):
    try:
        ...
        # Limpa o cache e tenta resetar a conexão se der erro (ex: banco dormindo)
        st.cache_data.clear()
        try:
            conn.reset() # Força o Streamlit a criar uma conexão nova
            return conn.query(query_str, params=params, ttl=ttl)
        except Exception:
            # Se falhar de novo, retorna um DataFrame vazio para não "crashar" o app
