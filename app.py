...
def tela_principal():
    ...
    if tipo == 'admin' and st.session_state.admin_mode: tela_admin()
        ...
        with t1:
            # BUSCA
            busca = st.text_input("🔍 Buscar Produtos", placeholder="Digite o nome do prêmio...").lower()
            df_p = run_query("SELECT * FROM premios ORDER BY id")
        ...
        # ABA 3: RESGATES CORRIGIDA (CASE INSENSITIVE)
        with t3:
            st.info("### 📜 Acompanhamento\nPedido recebido! Prazo: **5 dias úteis** no seu Whatsapp informado no momento do resgate!.")
            # CORREÇÃO: LOWER() PARA GARANTIR MATCH INDEPENDENTE DE MAIÚSCULAS/MINÚSCULAS
