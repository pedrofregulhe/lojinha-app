...
def converter_link_drive(url):
    ...
    if "drive.google.com" in url:
        if "id=" in url:
            # Já está no formato de ID
            return url
        try:
            # Extrai o ID de links no formato /file/d/ID/view
            file_id = url.split("/")[-2]
...
def processar_resgate(usuario_cod, item_nome, custo):
    try:
        ...
        df_u['u_s'] = df_u['usuario'].astype(str).apply(lambda x: limpar_dado(x).lower())
        idx = df_u[df_u['u_s'] == usuario_cod.lower()].index
        if len(idx) == 0: return False
        saldo_b = float(df_u.at[idx[0], 'saldo'])
        if saldo_b < custo: return False
        df_u.at[idx[0], 'saldo'] = saldo_b - custo
        conn.update(worksheet="usuarios", data=df_u.drop(columns=['u_s']))
...
def tela_principal():
    ...
    c_info, c_acoes = st.columns([3, 1.4])
    with c_info:
        st.markdown(f'<div class="header-style"><div style="display:flex; justify-content:space-between; align-items:center;"><div><h2 style="margin:0; color:white;">Olá, {u_nome}! 👋</h2><p style="margin:0; opacity:0.9;">Bem Vindo (a) a Loja Culligan.</p></div><div style="text-align:right;"><span style="font-size:12px; opacity:0.8;">SEU SALDO</span><br><span style="font-size:32px; font-weight:bold;">{sld:,.0f}</span> pts</div></div></div>', unsafe_allow_html=True)
    with c_acoes:
        img_b64 = carregar_logo_base64(ARQUIVO_LOGO)
    ...
    st.divider()
    if tipo == 'admin':
        tabs = st.tabs(["📊 Dashboard", "👥 Usuários", "🎁 Prêmios", "🛠️ Ferramentas"])
        with tabs[0]:
            df_v = carregar_dados("vendas")
            if not df_v.empty:
                st.data_editor(df_v, use_container_width=True, hide_index=True)
        with tabs[3]:
            sh = st.text_input("Senha para gerar Hash")
